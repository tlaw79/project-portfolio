...
struct Pose {
 public:
  ...
  /**
   * @brief Subtraction operator overloaded to calculate the distance 
   * between two Poses.
   *
   * @param other The second Pose object.
   *
   * @return Double representing the distance between two Poses.
   */
  double operator-(const Pose &rhs) const {
    double delta_x = this->x - rhs.x;
    double delta_y = this->y - rhs.y;
