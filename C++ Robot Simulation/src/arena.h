...
class Arena {
 public:
   ...
   *
   * @param quantity Number of Robots to add.
   * @param movement_type The movement type (Fear, Love etc.)
   */
  void AddRobot(int quantity, int movement_type);
  ...
  /**
   * @brief Add a sensor to the arena.
   *
   * @param robot The Robot whose sensor this is
   * @param side  Which side of Robot the sensor is on (Left or right)
   * @param type  The type of sensor.
   */
  void AddSensor(Robot * robot, int side, int type);

  /**
   * @brief Update sensor positions based on their Robot's position.
   */
  void UpdateSensorPositions();

  /**
   * @brief Update sensor readings based on their Robot's position.
   */
  void UpdateSensorReadings();

  /**
   * @brief Update the hunger timer for each Robot.
   * Robot becomes hungry after 30 seconds without eating.
   * Robot becomes really hungry after 2 minutes without eating.
   * Simulation ends if a Robot goes 2.5 minutes without eating.
   */
  void UpdateHungerTimers();

  /**
   * @brief Handles all collisions in the Arena.
   */
  void HandleCollisions();

  /**
   * @brief Accept communication from the controller.
  ...
  **/
  ...
  /**
   * @brief Converts the game status code into a string.
   *
   * @param game_status The game status code.
   * @param [out] A string corresponding to the code.
   */
  std::string ShowStatus(int game_status) {
    switch (game_status) {
      case(WON):
        return "You win!";
      case(LOST):
  ...
  bool get_paused_status() { return paused_status_; }
  void set_paused_status(bool status) { paused_status_ = status; }

  void set_num_robots(int n) { num_robots_ = n; }
  void set_num_lights(int n) { num_lights_ = n; }
  void set_num_food(int n) { num_food_ = n; }
  void set_ratio(float n) { ratio_ = n; }
  void set_sensitivity(int n) { sensitivity_ = n; }
  void set_food(bool food) { food_ = food; }
 ...
 private:
  ...
  // Ratio of fear Robots to explore Robots
  float ratio_{0.5};
